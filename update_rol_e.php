...
$key = $_SESSION["TipoUsuario"];
	...
	if ( !empty($_POST)) {
		...
		$valid = true;
		// if (empty($arol)) {
		// 	$arolError = 'Por favor seleccione un rol';
		// 	$valid = false;
		// }
		...
		if ($valid) {
			...
			if ($arol==1){
				$sql = 'UPDATE R_Alumno  set Matricula =?, Contrasenia =?, NombreAl =?, ApellidoAl =? WHERE Matricula = "' . $mail . '"';
				$q = $pdo->prepare($sql);
				...
				$sql2 = "DELETE FROM R_Alumno WHERE Matricula = ?";
				$q2 = $pdo->prepare($sql2);
				$q2->execute(array($id));
		...
		$pdo = Database::connect();
		$pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sql = "SELECT * FROM R_Alumno where Matricula = ?";
		$q = $pdo->prepare($sql);
		$q->execute(array($id));
	...
	<body>
    	<div class="container">
    		<div class="span10 offset1">
    			<div class="row">
		    		<h3>Actualizar Datos del Usuario/Estudiante</h3>
		    	</div>

	    			<form class="form-horizontal" action="update_rol_e.php?id=<?php echo $mail;?>" method="post">
				    	...
				    	<div class="controls">
	                       	<select name ="arol">
		                        <option value="">Selecciona un rol</option>
		                        	<option selected name="arol" value="1">Estudiante</option>
